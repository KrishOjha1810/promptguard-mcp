...
1. Flow-graph toxic-flow engine (taint graph: untrusted-source -> sensitive -> sink, multi-hop, cross-session) to replace the evadable 2-event pair. [L] DONE (single-session taint propagation): once any call touches sensitive data the session is tainted; any later external egress flags regardless of how many benign hops sit between (defeats the read/summarize/transform/send evasion). Egress broadened beyond send-named tools to any call carrying an external domain; internal/localhost destinations excluded. Severity: critical to a known exfiltration sink, high to an unknown external destination. Cross-session taint is still open (we see one trace at a time). 3 net new tests, 132 total.
2. Optional Ed25519 signing + external anchoring of the audit chain (turns tamper-evident into something that survives a compromised local process). [M] , the moat. DONE: `src/mcp-scan/signing.ts` (built-in crypto, no deps). `record --sign` generates/loads a local Ed25519 key (~/.promptguard, 0600) and signs each record over its hash (hash excludes the signature field). `verify --key` checks signatures, so a rewrite by someone without the key is caught even if they fix the hash chain. `anchor` prints a compact `pg-anchor:v1:...` token of the chain head; `verify --anchor <token>` catches a rewrite by someone WITH the key, because the recorded-elsewhere head no longer matches. Honest layering: chain = tamper-evident; signing = non-repudiable + blocks keyless forgery; anchoring = blocks keyed forgery if the anchor was recorded externally (e.g. a git commit). 8 tests, 129 total.
3. Entropy + structural secret detection layered on regex, with per-project allowlist learning to cut false positives. [M] DONE: `src/mcp-scan/entropy.ts` (Shannon entropy + base64/hex tokenizer, prose filter). Layered into the recorder behind `detectEntropy` (on in `record`, off in the base analyzeEvents so existing behavior is stable), and only when a named rule did not already fire, to avoid double-flagging. Catches encoded/unknown secrets the 17 named patterns miss; medium severity / 0.5 confidence since lower precision. `scan-mcp allow <token>` records a sha256 in ~/.promptguard/allowed-tokens.json so a benign high-entropy token is suppressed forever (the allowlist-learning suppressor). 7 tests, 139 total.
4. Behavioral sink scoring (novelty / first-seen destination / data-volume) instead of a static denylist, since webhook.site rotates and Anthropic MCP tunnels are now legitimate egress. [M] DONE: `record` keeps a local destinations store (~/.promptguard/destinations.json); tainted data going to a never-before-seen destination scores critical, to a previously-seen one scores high. Denylist remains an additional always-critical signal. `--no-memory` disables. Novelty, not a blocklist. 1 test, 133 total.
5. Two-fetch / scheduled re-pin to close the between-session rug-pull gap. [S] PARTIAL: the "scheduled re-pin" half is already covered by the always-on SessionStart monitor (R3), which re-checks every pinned tool every session and surfaces drift. The "two-fetch at connect with a delay" half requires a live MCP connection to fetch a server's tool list twice, which cannot be built or tested in this offline environment; deferred until there is a live-connection path. Documented honestly rather than shipping an untestable stub.
